...
class Packages extends CI_Controller {
	...
	function main()
	{
		...
		$data['packages'] = $this->Chart_model->get_packages(); // Memuat data dari model
        ...
        function listcol(){
            ...
            $colModel['device'] = array('Device',110,TRUE,'left',2);
            $colModel['name'] = array('Name',110,TRUE,'left',2);
            $colModel['code'] = array('Kode',110,TRUE,'left',2);
            $colModel['kode_produk_ax'] = array('Kode Produk AX',110,TRUE,'left',2);
            $colModel['method'] = array('Type',110,TRUE,'left',2);
            $colModel['consumer'] = array('Consumer',110,TRUE,'left',2);
            $colModel['jenjang_'] = array('Jenjang',110,TRUE,'left',2);
            $colModel['periode'] = array('Duration',110,TRUE,'left',2);
            $colModel['price_'] = array('Price',110,TRUE,'left',2);
            $colModel['status_'] = array('Status',110,TRUE,'left',2);
	...
	function get_flexigrid()
        {
            ...
            $this->db->select("a.id idnya,a.*,b.title duration_,c.title jenjang_,FORMAT(a.price,0) as price_,case a.status
			when '0' then 'Tidak Aktif'
			when '1' then 'Aktif'
			end as status_",false)->from($this->utama." a");
            $this->db->join("setup_periode b","a.periode=b.id","left");
	...
	function form($id=null){
		...
		$data['opt_jenjang']=GetOptAll('labels','All Video',array('parent'=>'where/1','deletedAt'=>'where_is_null/1'));
...
//		else{$data['is_active']='Active';}
		$a = new stdClass();
		foreach($this->input->post('description') as $dsc){
			$a->desc[]=$dsc;
		}
		$data['description']=json_encode($a);
		if($id != NULL && $id != '')
		{
