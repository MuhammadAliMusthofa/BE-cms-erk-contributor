...
class Videos extends CI_Controller {
	...
	function main()
	{
			...
			$data['get_video']= $this->Chart_model->get_videos_with_labels_combined();
	...
	public function get_videos_with_labels_combined() {
        ...
        // Panggil method dari model untuk mengambil data videos dengan label yang digabung
        $videos = $this->Chart_model->get_videos_with_labels_combined();
	...
	function get_record(){
			$query = $this->db->query('SELECT * FROM sv_videos LIMIT 2');
			$colModel = $query->result();
            $colModel=$this->listcol();
            $z=0;
	...
	function form_worksheet($id = null, $lang) {
		izin();
		if ($lang == 'en' || $lang == 'id') {
			if ($id != NULL) {
				$filter = array('id' => 'where/'.$id);
			...
			$data['lang'] = $lang;
			$data['content'] = 'contents/'.$this->utama.'/edit_worksheet';
	...
	function submit_worksheet() {
		...
		if ($id != NULL && $id != '') {
			// $file_worksheet = $this->input->post('video_id');
			$file_worksheet = date("mdYHis");;
	
			// $file_worksheet_url = base_url() . 'assets/upload_worksheet/' . $file_worksheet . '.pdf';
	
			$data = array(
				// 'video_id' => $this->input->post('video_id'),
				'file_worksheet' => $file_worksheet. '.pdf',
				'updatedBy' => $webmaster_id,
			...
			$this->session->set_flashdata("err_code", '0');
			$this->session->set_flashdata("message", 'Worksheet Berhasil di Upload');
        ...
        foreach($this->input->post('tags') as $tag){
            ...
            if(!empty($id) && $id>0){
			    ...
			    if(substr($insertvideotobc_id['httpcode'],0,1)==2 && substr($insertvideotobc_en['httpcode'],0,1)==2){
				    ...
				    $this->session->set_flashdata("message", json_encode($insertvideotobc_id));
                    redirect($this->utama);
	...
	function upload_worksheet() {
		$cpDir = './assets/upload_worksheet/';
		if (!is_dir($cpDir)) {
	...
	public function data_ar()
    {
        $Videos = new Chart_model();
        $data = $Videos->get_video_ar();
        echo json_encode($data);
    }

	function edit_ar($id=null,$lang){
		izin();
		if($lang=='en' || $lang=='id'){
		if($id!=NULL){
			$filter=array('id'=>'where/'.$id);
		...
		$data['lang']=$lang;
		$data['content'] = 'contents/'.$this->utama.'/edit_ar';
	...
	function submit_ar(){
		...
		// Menambahkan data repository dan project ke dalam $data
		$data['repository'] = $this->input->post('repository');
