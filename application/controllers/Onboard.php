...
class Onboard extends CI_Controller {
	...
	function main()
	{
		...
		$data['onboard'] = $this->Chart_model->getOnboard();
        ...
        function listcol(){
            ...
            $colModel['title_id'] = array('Title (ID)',250,TRUE,'left',2);
            $colModel['status'] = array('Status',200,TRUE,'left',2);
	...
	function get_record(){
            ...
            foreach ($records['records']->result() as $row)
            {
				...
				foreach($colModel as $key=>$cm){
					if($key=='status'){
                        $record_items[$a][$b]="<span style='color:$coloring'>".($row->$key==0 ? 'Inactive':'Active' )."</span>";
						$b++;
					}
                    elseif($key!='id' && $key!='status'){
...
//		else{$data['is_active']='Active';}
		...
		if (!empty($_FILES['filez_id']['name'])) {
			$time=date('YmdHis');
			$config['upload_path'] = './assets/upload_onboard/';
			...
			if(!empty($id)){
				unlink('./assets/upload_onboard/'.GetValue('image','sv_'.$this->utama,array('id'=>'where/'.$id)));
			}
			...
			if (!$this->upload->do_upload('filez_id')) {
				...
				//$data['image']=base_url().'assets/upload_contents/'.$file_info['file_name'];
