...
class Vocmanualexp extends CI_Controller {
	
		var $utama ='vocmanualexp';
		var $title ='Voucher Manual Expired';
	...
	function main()
	{
		...
		$data['vouchers'] = $this->Chart_model->getListVoucher();
		// $data['vouchers'] = $this->Chart_model->getVouchers();
		$expiredCount = $this->Chart_model->getExpiredCount(); 
    	$data['expiredCount'] = $expiredCount;
		
		$this->load->view('layout/main',$data);
	}
	
	public function getListVoucherAjax() {
		$vouchers = $this->Chart_model->getListVoucher(); // Ganti Your_model_name dengan nama model Anda
    ...
    {
        $Vouch = new Chart_model();
        $data = $Vouch->getVouchExp();
        echo json_encode($data);
    }
	public function approachingExpiredCount() {
        $count = $this->Chart_model->approachingExpiredCount();
        echo $count; // Output jumlah data yang akan kedaluwarsa dalam 3 hari
    }
	

	public function updateexp($id) {
        // Pastikan ID yang diterima adalah angka dan bukan string berbahaya
        if (!is_numeric($id)) {
            show_error('Invalid ID');
        }

        // Load model yang sesuai untuk mengelola data
        $this->load->model('Chart_model');

        // Lakukan pengambilan data berdasarkan ID
        $record = $this->Chart_model->getRecordById($id);
        ...
        if ($this->input->post()) {
            $expiredAt = $this->input->post('expiredAt');
            // Lakukan proses update expiredAt ke dalam database melalui model
            $update_data = array(
                'expiredAt' => $expiredAt,
                'updatedAt' => date('Y-m-d H:i:s')
            );
            $this->Chart_model->updateRecord($id, $update_data);

            // Redirect ke halaman yang sesuai atau tampilkan pesan sukses
            redirect($this->utama);
        }

        // Jika validasi gagal atau belum ada input form, tampilkan kembali form untuk diedit
        $data['record'] = $record;
        $this->load->view('vocmanualexp', $data);
    }
