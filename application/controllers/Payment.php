...
class Payment extends CI_Controller {
	...
	function main()
	{
		...
		$data['payments'] = $this->Chart_model->getPaymentDetails();
	...
	{
		$transactionStatus = $this->input->post('edit_transaction_status');
		...
		if (!empty($transactionStatus)) {
			...
			$this->load->model('Chart_model');
			$this->Chart_model->update_transaction($id, $data);
		}
	...
	public function update_tr($id)
	{
		...
		if ($this->input->post()) {
			...
			$datatr = array(
				'transaction_status' => $this->input->post('edit_transaction_status'),
			);

			if ($datatr['transaction_status'] === 'settlement') {
				$datatr['settlement_time'] = time();
			}

			if ($this->input->post('transaction_time') !== null) {
				$datatr['transaction_time'] = strtotime($this->input->post('transaction_time'));
			}

			$this->Chart_model->update_transaction($id, $datatr);

			if (isset($datatr['settlement_time'])) {
				$datausr = array(
				...
				$this->Chart_model->manualactivation($id, $datausr);

				echo '<script>alert("Status Aktivasi Berhasil"); window.history.back();</script>';
				exit;
				// redirect($this->utama);
			} else {
				echo '<script>alert("Status Transaksi Berhasil Diubah"); window.history.back();</script>';
				exit; 
			}
		} else {
			// redirect($this->utama);
	...
	public function insert_userpakage($id)
	{
		if ($this->input->post()) {
			$data = array(
				'transaction_status' => $this->input->post('edit_transaction_status'),
			...
			$this->load->model('Chart_model');
			$this->Chart_model->userpakage_transaction($data);
